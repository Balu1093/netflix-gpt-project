...
const ShowInfo = ({test,setTest,props,data}) => {
  ...
  return (
    <div className='bg-[rgba(49,49,49,0.7)] fixed w-[100%] h-[100%] top-0 left-0 right-0 bottom-0 z-10 text-white'>
        ...
        <div className='absolute w-[100%] sm:w-[90%] md:w-[80%] lg:w-[70%] 2xl:w-[60%] 2xl:h-[900px] top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 py-4 px-4 min-h-[900px]'>
        <div className='relative top-16 bg-gradient-to-tl from-slate-600 to-slate-900 w-full h-screen rounded-lg'>
        ...
        <div className='absolute w-[90%] sm:w-[90%] md:w-[80%] lg:w-[80%] 2xl:w-[80%] mx-auto top-[10%] sm:top-[10%] md:top-0 bottom-0 left-0 right-0 md:flex lg:flex 2xl:flex items-center'>
            <div className='mx-auto sm:mx-auto md:ml-0 w-[250px] sm:w-[250px] md:w-[300px] 2xl:w-[400px] h-[200px] sm:h-[200px] md:h-[400px] lg:h-[500px] 2xl:h-[600px] rounded-lg'>
            ...
            <div className='flex flex-col gap-2 ml-8 sm:ml-8 md:ml-10 lg:ml-12 2xl:ml-14 12/12 md:w-7/12 lg:w-7/12 2xl:w-7/12'>
              <h1 className='text-xl 2xl:text-5xl lg:text-4xl md:text-3xl sm:text-xl font-semibold'>{movieInfo.title}</h1>
              <h3 className='text-[12px] sm:text-[14px] md:text-[18px]'>{movieInfo.release_date} <span className='font-bold text-2xl'>.</span> {movieInfo.genres.map((x,index)=><span>{(index?', ':'')+ x.name}</span>)} <span className='font-bold text-2xl'>.</span> <span>{timeConvert(movieInfo.runtime)}</span></h3>
              ...
              <div className='h-5 md:h-6 w-5 md:w-6 ml-6 group cursor-pointer relative text-[12px] sm:text-[14px] md:text-[18px]' onClick={addFavourite}>
                ...
                <div className='absolute hidden group-hover:block -top-[50%] md:-top-[70%] left-7 w-max bg-white bg-opacity-45 py-[12px] md:py-[18px] px-[16px] md:px-[22px] z-10 rounded-full'><button className='font-semibold text-red cursor-pointer'>Add to Favourite</button></div>
              </div>
               </div>
               <div className='md:my-2 lg:my-3 2xl:my-4'>
               <h1 className='text-white font-semibold opacity-70 text-[14px] sm:text-[16px] md:text-xl'>{movieInfo.tagline}</h1>
               </div>
               <div>
                <h3 className='text-[14px] lg:text-[16px] 2xl:text-xl font-semibold'>OverView</h3>
                <h4 className='text-[12px] sm:text-[14px] md:text-[18px] mt-2'>{movieInfo.overview}</h4>
               </div>
               <div className='flex mt-3 md:mt-3 lg:mt-4 gap-10'>
  ...
  <div className=''>
            <button className='absolute text-white top-[10%] end-6 py-2 px-2 font-medium text-xl bg-white rounded-full bg-opacity-40' onClick={buttonClick}> <IoCloseSharp/></button>
  </div>
  </div>
  </div>
  )
